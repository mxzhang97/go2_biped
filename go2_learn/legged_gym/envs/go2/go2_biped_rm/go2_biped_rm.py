...
from .go2_biped_rm_config import Go2BipedRMCfg
...
class Go2BipedRM(BaseTask):
    cfg: Go2BipedRMCfg

    def __init__(self, cfg: Go2BipedRMCfg, sim_params: dict, sim_device: str, headless: bool):
    ...
    def compute_observations(self):
        ...
        self._check_nan(self.com_pos, name="com_pos", rigid_body_pos = rigid_body_pos, mass = self.simulator._mass.unsqueeze(-1))
        self._check_inf(self.com_pos, name="com_pos", rigid_body_pos = rigid_body_pos, mass = self.simulator._mass.unsqueeze(-1))
    ...
    def compute_reward(self):
            ...
            if self.cfg.rewards.stage_2:
                gait_bonus = self._compute_gait_reward_and_update_rm(self.rew_buf)
                self.episode_sums["gait_bonus"] += torch.where(gait_bonus> 1.0, 1.0, 0.0)
                self.rew_buf *= gait_bonus
...
# callbacks
    ...
    def _get_noise_scale_vec(self):
        ...
        noise_vec[9:12] = noise_scales.com_cop * noise_level 
    ...
    def _compute_gait_reward_and_update_rm(self, locomotion_rewards):
        ...
        final_rewards = locomotion_rewards.clone()
        bonus_factor = torch.ones_like(final_rewards)
        apply_bonus_mask = all_transitions & (locomotion_rewards > 0)
        bonus_factor[apply_bonus_mask] = self.cfg.rewards.gait_transition_bonus
        
        final_rewards *= bonus_factor
    ...
    def _reward_lin_vel_z(self):
        ...
        return torch.square(self.simulator.base_lin_vel[:, 0])

    def _reward_ang_vel_xy(self):

        return torch.sum(torch.square(self.simulator.base_ang_vel[:, 1:3]), dim=1)

    def _reward_orientation(self):

        return torch.sum(torch.square(self.simulator.projected_gravity[:, :2]), dim=1) * torch.clamp(-self.simulator.projected_gravity[:,0], min = 0.0)

    def _reward_orientation_3(self):

        return torch.sum(torch.square(self.simulator.projected_gravity[:, 2]).unsqueeze(1), dim=1)
    ...
    def _reward_fl_arm_track(self):
        ...
        return torch.exp(-torch.square(clamped_dist)/0.05)  * orientation_check
    ...
    def _reward_fl_arm_track_lateral_distance(self):
        ...
        return (torch.square(lateral_dist-0.18) + torch.square(vertical_dist - 0.15))  * orientation_check
    ...
    def _reward_rear_hip_joint(self):
        ...
        return torch.sum(torch.square(dof_diff), dim=-1)
